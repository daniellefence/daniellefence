<x-page-heading :model="$product">
    <x-slot name="heading">{{$product->title}}</x-slot>
    <x-slot name="subheading">{{$product->category->title}}</x-slot>
</x-page-heading>

<div class="pb-16 pt-6 sm:pb-24 bg-gradient-to-br from-gray-50 to-white">
            ...
            <div class="mt-8 lg:col-span-7 lg:col-start-1 lg:row-span-3 lg:row-start-1 lg:mt-0">
                <div class="bg-white rounded-2xl shadow-lg p-6 border border-outdoor-light">
                    ...
                    <p class="text-center text-base text-slate-600 mt-4 flex items-center justify-center gap-2 bg-outdoor-light/30 rounded-lg py-3 px-4">
                        <i class="fa-solid fa-magnifying-glass-plus w-5 h-5"></i>
                    ...
                    <div class="mt-10 flex flex-col items-center gap-4">
                        @if($product->pip()->count() >0)
                            <div class="hidden sm:block bg-gray-50 rounded-lg p-4">
                                <embed class="product-embed-dimensions" src="{{asset('storage/'.$product->pip->path)}}#toolbar=0&navpanes=0&scrollbar=0&page=1" alt="{{$product->title}}"/>
                            </div>
